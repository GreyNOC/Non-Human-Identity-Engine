...
def parse(path: Path, text: str) -> list[Signal]:
    ...
    for match in WEBHOOK_RE.finditer(text):
        ...
        signals.append(make_signal(rule_id="nhi_webhook_secret_exposed", file_path=path, line_number=None, name="Webhook URL", identity_type="webhook secret", source="webhook parser", evidence=url, secret_value=url, provider=provider, external_access=True, tags=["webhook", "plaintext_secret"]))
