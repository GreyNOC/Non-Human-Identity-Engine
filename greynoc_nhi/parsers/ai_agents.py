...
def _tool_specs(path: Path, text: str, rows: list[tuple[str, object, int | None]]) -> list[dict[str, str]]:
    data = parse_json_safely(text)
    if data is not None:
...
def _tool_line(text: str, spec: dict[str, str]) -> int | None:
    name = spec.get("name", "")
    description = spec.get("description", "")[:40]
    return (line_number_for(text, name) if name else None) or (line_number_for(text, description) if description else None)
...
def parse(path: Path, text: str) -> list[Signal]:
    ...
    tool_specs = _tool_specs(path, text, rows)
        ...
        if TOOL_PROMPT_INJECTION_RE.search(description):
            signals.append(
                make_signal(
                    ...
                    file_path=path,
                    line_number=_tool_line(text, spec),
                    name=name,
        ...
        if len(specs) >= 2 and (len(sources) >= 2 or len(descriptions) >= 2 or any(any(marker in src.lower() for marker in UNTRUSTED_TOOL_SOURCE_MARKERS) for src in sources)):
            signals.append(
                make_signal(
                    ...
                    file_path=path,
                    line_number=_tool_line(text, specs[0]),
