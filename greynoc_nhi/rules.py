...
RULE_CATALOG: dict[str, RuleTemplate] = {
    ...
    "nhi_linux_pam_unknown_module": RuleTemplate("nhi_linux_pam_unknown_module", "Unknown PAM module detected", 75, "Linux auth persistence", "A PAM module is not in the built-in common module list.", "Unexpected PAM modules can intercept authentication or provide durable host persistence.", "Verify module provenance, owner, package source, checksum, and whether it is required.", ["host auth inventory", "module provenance"]),
    "nhi_linux_pam_absolute_module_path": RuleTemplate("nhi_linux_pam_absolute_module_path", "PAM config references absolute module path", 75, "Linux auth persistence", "A PAM auth chain references a module by absolute path.", "Absolute paths can bypass distro-managed PAM module lookup and hide custom persistence.", "Move required modules into trusted package-managed security directories or remove the reference.", ["PAM hardening", "path review"]),
    "nhi_linux_pam_module_outside_trusted_dir": RuleTemplate("nhi_linux_pam_module_outside_trusted_dir", "PAM module is outside trusted security directories", 90, "Linux auth persistence", "A PAM config or script references a module outside trusted security module paths.", "Modules outside trusted directories are a common persistence and credential-interception path.", "Remove the module, inspect the host for compromise, and rebuild auth configuration from trusted packages.", ["PAM hardening", "incident review"]),
    "nhi_linux_pam_module_world_writable": RuleTemplate("nhi_linux_pam_module_world_writable", "PAM module is world-writable", 95, "Linux auth persistence", "A PAM shared object can be modified by unprivileged users.", "World-writable auth modules let local users alter login behavior or harvest credentials.", "Restore owner-only permissions, reinstall the package, and investigate prior modifications.", ["file permissions", "host integrity"]),
    "nhi_linux_pam_config_world_writable": RuleTemplate("nhi_linux_pam_config_world_writable", "PAM or SSH auth config is world-writable", 95, "Linux auth persistence", "A PAM/SSH authentication configuration file can be modified by unprivileged users.", "Writable auth configs let local users change login policy or add credential-harvesting modules.", "Restore root-owned permissions and review recent changes to the auth chain.", ["file permissions", "host integrity"]),
    "nhi_linux_pam_referenced_module_missing": RuleTemplate("nhi_linux_pam_referenced_module_missing", "PAM config references missing module", 60, "Linux auth integrity", "A PAM auth chain references a module that was not found in inspected security paths.", "Missing auth modules can indicate broken hardening, failed cleanup, or tampered host state.", "Validate package state and remove stale or malicious references.", ["PAM integrity", "package verification"]),
    "nhi_linux_pam_auth_chain_modified": RuleTemplate("nhi_linux_pam_auth_chain_modified", "PAM/SSH authentication chain modified", 85, "Linux auth persistence", "A file or script appears to edit PAM/SSH authentication behavior.", "Auth-chain modifications can persist across restarts and affect all SSH or local logins.", "Review the change, require host-owner approval, and compare against the expected distro baseline.", ["change control", "PAM review"]),
    "nhi_linux_pam_ssh_credential_theft_indicators": RuleTemplate("nhi_linux_pam_ssh_credential_theft_indicators", "PAM/SSH credential-theft indicators detected", 95, "Linux auth credential theft", "PAM/SSH code or strings reference credential, SSH session, or auth-token material.", "Malicious PAM modules often harvest SSH passwords, tokens, or session metadata during login.", "Isolate the host, preserve evidence, rotate affected credentials, and rebuild PAM from trusted packages.", ["incident response", "credential rotation"]),
    "nhi_linux_pam_antiforensics_indicators": RuleTemplate("nhi_linux_pam_antiforensics_indicators", "PAM/SSH anti-forensics indicators detected", 90, "Linux auth anti-forensics", "PAM/SSH-adjacent code or strings reference shell history or login-record tampering.", "Anti-forensics around auth paths suggests an attempt to hide interactive access or persistence.", "Investigate login records, audit logs, and host integrity before trusting the system.", ["incident response", "audit logs"]),
    "nhi_linux_pam_persistence_path": RuleTemplate("nhi_linux_pam_persistence_path", "PAM persistence path detected", 85, "Linux auth persistence", "A script, config, or host file references a PAM module installation or execution path.", "PAM module paths are high-impact because they execute during authentication flows.", "Confirm the module is package-managed, owned by root, permissioned safely, and reviewed.", ["PAM hardening", "module provenance"]),
}
...
def needs_identity_governance_finding(identity: NonHumanIdentity) -> bool:
    ...
    return (
        ...
        or identity.identity_type in {"ci_runner", "deployment_identity", "service_account", "mcp_server", "ai_agent", "tool_connector", "model_gateway", "linux_auth_module"}
    )
